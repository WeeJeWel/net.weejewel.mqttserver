...
module.exports = class MQTTServerApp extends Homey.App {
  ...
  async onInit() {
    ...
    await new Promise((resolve, reject) => {
      ...
      this.server.listen(PORT_MQTT, err => {
        if (err) return reject(err);
        return resolve();
      });
    })
      .then(() => {
        this.log(`Listening on mqtt://0.0.0.0:${PORT_MQTT}`);
      })
      .catch(err => {
    ...
    await new Promise((resolve, reject) => {
      ...
      this.serverSecure.listen(PORT_MQTTS, err => {
        if (err) return reject(err);
        return resolve();
      });
    })
      .then(() => {
        this.log(`Listening on mqtts://0.0.0.0:${PORT_MQTTS}`);
      })
      .catch(err => {
  ...
  publishDevice(device) {
    ...
    this.publish(`homie/${device.id}/$homie`, '3.0');
    this.publish(`homie/${device.id}/$name`, String(device.name));
    this.publish(`homie/${device.id}/$state`, device.available ? 'ready' : 'disconnected');
  }
  ...
  publishDeviceCapability(device, capabilityId, capabilityObj, value) {
    ...
    this.publish(`homie/${device.id}/main/${capabilityId}`, String(value));
    this.publish(`homie/${device.id}/main/${capabilityId}/$name`, String(capabilityObj.title));
    this.publish(`homie/${device.id}/main/${capabilityId}/$unit`, String(capabilityObj.units ?? ''));
    this.publish(`homie/${device.id}/main/${capabilityId}/$datatype`, HomieUtil.homeyTypeToHomieDatatype(capabilityObj.type));
    this.publish(`homie/${device.id}/main/${capabilityId}/$settable`, String(true));
  }
  ...
  onPublish(packet, client) {
    ...
    Promise.resolve().then(async () => {
      const regex = new RegExp('homie/(?<deviceId>[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})/main/(?<capabilityId>[A-Za-z0-9._-]+)/set');
      const parsed = regex.exec(packet.topic);
      if (!parsed) return;
