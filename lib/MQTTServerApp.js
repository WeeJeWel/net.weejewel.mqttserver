...
module.exports = class MQTTServerApp extends Homey.App {
  ...
  async onInit() {
    ...
    await this.homey.flow
      .getActionCard('publish-topic')
      .registerRunListener(async ({ topic, value }) => {
        await this.publish(topic, value);
      });

    await this.homey.flow
      .getActionCard('unpublish-topic')
      .registerRunListener(async ({ topic }) => {
        await this.unpublish(topic);
      });
  }
